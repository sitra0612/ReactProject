...
import styles from "./Component/About/about.module.css";
...
function App() {
  ...
  {/* <div>
    {
    //   comment.length <=0 ?  (<h3>No comments</h3>) : (  comment.map((val,idx)=>{
    ...
    comment.map((val,idx)=>{
        ...
        <div class={styles['team-cards']}>
          ...
          <div className={styles['card-info']} key={idx}>
            <h2 className={styles['card-name']} key={idx}>Almaz</h2>
            <p className={styles['card-role']} >{ val}</p>
  ...
  <button onClick={addComment}>POST</button><button className={styles.button} >reply</button> */}
